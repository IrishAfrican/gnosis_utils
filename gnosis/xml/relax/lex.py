...
class Lexer:
    ...
    # ------------------------------------------------------------
    def input(self,s):
        if not isinstance(s,types.bytes):
...
def lex(module=None,debug=0,optimize=0,lextab="lextab"):
    ...
    if not (isinstance(tokens,types.list) or isinstance(tokens,tuple)):
    ...
    if debug:
        print("lex: tokens = '%s'" % list(lexer.lextokens.keys()))
    ...
    for f in tsymbols:
        ...
        elif isinstance(ldict[f],types.bytes):
    ...
    try:
        ...
        for f,i in list(lexer.lexre.groupindex.items()):
            handle = ldict[f]
