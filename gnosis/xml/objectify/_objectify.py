...
def children(o):
    ...
    return [x for x in content(o) if type(x) not in bytess]
def text(o):
    "List of textual children"
    return [x for x in content(o) if type(x) in bytess]
def dumps(o):
...
def attributes(o):
    ...
    return [(k,v) for k,v in list(o.__dict__.items())
                  if k!='PCDATA' and type(v) in bytess]
...
class XML_Objectify:
    ...
    def __init__(self, xml_src=None, parser=EXPAT):
        ...
        if parser==DOM and (hasattr(xml_src,'documentElement')
            ...
            self._dom = xml_src
            self._fh = None
        elif type(xml_src) in (bytes, str):
